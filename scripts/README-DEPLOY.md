...
We will use **InternalWebServer** template as and example of use (we suppose it was created and is already published in AD CS):
...
# We use covadonga-srv as an example of use (note & character at the end of the first DNS item in the list)
@"
[NewRequest]
...
# Submit the request
# For example: certreq -submit -config "TORROX-SRV\asturmalaga-TORROX-SRV-CA" -attrib "certificateTemplate:InternalWebServer" certreq.cer certreq.crt
# Warning: Use template name (do not use template display name)
certreq -submit -config "CA-SERVER\<CA-AUTH-NAME>" -attrib "certificateTemplate:<TEMPLATE-NAME>" certreq.cer certreq.crt
...
# For example: Get-ChildItem Cert:\LocalMachine\My | Where-Object { $_.Subject -like "*covadonga-srv*" }
...
```powershell
# Set the HTTPS URL
# For example: [Environment]::SetEnvironmentVariable("ASPNETCORE_URLS", "https://+:5001", "Machine")
[Environment]::SetEnvironmentVariable("ASPNETCORE_URLS", "https://+:<PORT>", "Machine")

# Bind the certificate to the port using appsettings.json
"Kestrel": {
    "Certificates": {
      "Default": {
        "Subject": <SERVER-FQDN WITHOUT CN= PREFIX> # For example: "covadonga-srv.asturmalaga.com",
        "Store": "My",
...
### Step 5 — Create and check SPNs for HTTPS
...
- It must not include the port.
  The client (browser, HttpClient, etc.) requests the Kerberos ticket for `HTTP/<SERVER-FQDN>` without `:<PORT>`,   
  regardless of the actual connection port.

```powershell
# For example: setspn -S HTTP/covadonga-srv.asturmalaga.com asturmal\administrador
setspn -S <PROTOCOL>://<SERVER-FQDN> DOMAIN\USER  

# For example: setspn -Q HTTP/covadonga-srv.asturmalaga.com
setspn -S <PROTOCOL>/<SERVER> DOMAIN\USER  

# For example: setspn -Q HTTP/covadonga-srv.asturmalaga.com
setspn -Q <PROTOCOL>/<SERVER-FQDN>

# For example: setspn -L asturmal\administrador
...
New-NetFirewallRule -DisplayName "Test-IA.WebApp HTTPS" `
    -Direction Inbound `
    -Protocol TCP `
    -LocalPort <PORT> ` # For example: 5001
    -Action Allow
```
...
# For example: Invoke-WebRequest -Uri "https://covadonga-srv.asturmalaga.com:5001" -UseBasicParsing -UseDefaultCredentials
Invoke-WebRequest -Uri <PROTOCOL>://<SERVER-FQDN>:<PORT> -UseBasicParsing -UseDefaultCredentials

# Verify the certificate chain
# For example: Invoke-WebRequest -Uri "https://covadonga-srv.asturmalaga.com:5001" -UseBasicParsing -UseDefaultCredentials | Select-Object -ExpandProperty Content
Invoke-WebRequest -Uri <PROTOCOL>://<SERVER-FQDN>:<PORT> -UseBasicParsing -UseDefaultCredentials | Select-Object -ExpandProperty Content
```
...
```
# For example: https://covadonga-srv:5001     1  
<PROTOCOL>://<SERVER>:<PORT>      1  

https://covadonga-srv.asturmalaga.com:5001    1  
<PROTOCL>://<SERVER-FQDN>:<PORT>   1
```
