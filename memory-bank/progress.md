...
- **WebApp**: ASP.NET Core Razor Pages application with:
  ...
  - **Groups.cshtml / Groups.cshtml.cs**: Dedicated Razor Pages for group management. Supports group search and **adding members to groups** via `IGroupMembershipWriter`. Dynamic display via `IAttributeMapper<GroupDto>`.icy-based authorization**: `AddPolicy("RequiredGroup")` with `GroupAuthorizationHandler` using `IServiceScopeFactory` for scoped service resolution.
